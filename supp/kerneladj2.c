...
/*
# Usage in R:
kerneladj2 <- function(genotype, pht, r.max = 10^7, k = 10L, offset, sigma) 
{ 
    storage.mode(genotype) <- "integer"
    .Call("_kerneladj2", as.matrix(genotype), as.integer(pht), as.numeric(r.max), as.integer(k), as.integer(offset), as.numeric(sigma))
}
*/

SEXP _kerneladj2(SEXP genotype, SEXP phenotype, SEXP rmax, SEXP k, SEXP offset, SEXP sigma)
{
    ...
    row_coeff = malloc((size_t) ncol * sizeof *row_coeff);
    
    if (!(permut && row_count && row_tablesum && row_filter && row_sums && row_unique && row_remap && row_coeff)) goto error2; // Memory allocation failed
    ...
    for (unsigned j = 0; j < ncol; j++)
    {
        ...
        // Computing p-values
        row_coeff[j] = Rf_dnorm4(INTEGER(offset)[j], 0, REAL(sigma)[0], 0);
        ...
        if (outer_min <= 4. && cdx == 2 && cdy == 2)
        {
            ...
            density += (log(hyp_sum) - log(hyp_less)) * row_coeff[j];
        }
        else
        {
            double stat = 0.;
            ...
            density -= Rf_pchisq(stat, (cdx - 1) * (cdy - 1), 0, 1) * row_coeff[j]; // Upper tail and logarithm are enabled
        }
    }
    ...
    for (; rpl < rbnd && (fixed || qc <= qcbnd); rpl++)
    {
        ...
        for (unsigned j = 0; j < ncol; j++) if (row_count[j] && row_unique[j])
        {
            ...
            if (outer_min <= 4. && cdx == 2 && cdy == 2)
            {
                ...
                density_ += (log(hyp_sum) - log(hyp_less)) * row_coeff[j];
            }
            else
            {
                double stat = 0.;
                ...
                density_ -= Rf_pchisq(stat, (cdx - 1) * (cdy - 1), 0, 1) * row_coeff[j]; // Upper tail and logarithm are enabled
            }
        }
...
epilogue:
    ...
    error1:
        Rf_warning("Function '%s' exited with an error: invalid array of coordinate offsets!\n", __FUNCTION__);
        break;
