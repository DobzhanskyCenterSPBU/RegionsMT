...
SEXP _maxadj2(SEXP genotype, SEXP phenotype, SEXP rmax, SEXP k)
{
    ...
    if (!(permut && row_count && row_tablesum && row_filter && row_sums && row_unique && row_remap)) goto error1; // Memory allocation failed
    ...
    for (unsigned j = 0; j < ncol; j++)
    {
        ...
        else
        {
            ...
            val = Rf_pchisq(stat, (cdx - 1) * (cdy - 1), 0, 0);
        }
        
        if (val < density) density = val;
    }
