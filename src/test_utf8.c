...
bool utf8_test(void *dst, size_t *p_context)
{
    ...
    size_t context = *p_context;
    if (context < countof(data)) memcpy(dst, data + context, sizeof(*data)), ++*p_context;
    else *p_context = 0;
    return 1;
}
...
bool utf8_test_len(void *In, struct message *p_message)
{
    ...
    if (len != in->utf8_len) *p_message = MESSAGE(NULL, MESSAGE_TYPE_DEFAULT);
    else return 1;
    return 0;
}
