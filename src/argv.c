        ...
        if (!halt)
        {
            ...
            else if (argv[i][0] == '-')
            {
                if (argv[i][1] == '-') // Long mode
                {
                    ...
                    if (!selector(&par, str, len, 0, context)) 
                ...
                else // Short mode
                {
                    ...
                    for (size_t k = 1; argv[i][k];) // Inner loop for handling multiple option-like parameters
                    {
                        ...
                        {
                            len = utf8_len;
                            if (!selector(&par, str, len, 1, context))
                                log_message(log, &MESSAGE_WARNING_ARGV_PARSE(str, len, i, ARGV_WARNING_INVALID_PAR).base);
                            else
                            {
...
bool argv_par_selector(struct par *par, char *str, size_t len, bool shrt, void *Context)
{
    struct argv_par_sch *context = Context;
    size_t id = shrt ?
        binary_search(str, context->stag, sizeof(*context->stag), context->stag_cnt, str_strl_cmp_len, &len) :
        binary_search(str, context->ltag, sizeof(*context->ltag), context->ltag_cnt, len + 1 ? str_strl_cmp_len : str_strl_cmp, &len);
    if (id + 1)
    {
        id = (shrt ? context->stag : context->ltag)[id].id;
